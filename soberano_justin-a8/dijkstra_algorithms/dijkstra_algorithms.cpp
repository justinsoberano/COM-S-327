...
int dijkstra(Point start, Point end) {
    ...
    dist[start.get_position_x()][start.get_position_y()] = 0;

    Node queue[ROWS * COLS];
    int front = 0, rear = 0;
    queue[rear++] = (Node){start.get_position_x(), start.get_position_y(), 0};
    ...
    if (dist[end.get_position_x()][end.get_position_y()] == INT_MAX) {
        return -1;
    }

    Point curr = end;
    while (curr.get_position_x() != start.get_position_x() || curr.get_position_y() != start.get_position_y()) {
        earth[LY][LX].map[curr.get_position_x()][curr.get_position_y()] = PATH;
        curr = (Point){prev[curr.get_position_x()][curr.get_position_y()].x, prev[curr.get_position_x()][curr.get_position_y()].y};
    }
    earth[LY][LX].map[start.get_position_x()][start.get_position_y()] = PATH;
    earth[LY][LX].map[end.get_position_x()][end.get_position_y()] = PATH;

    return dist[end.get_position_x()][end.get_position_y()];
}
...
int dijkstra_for_pc(Point start, CharacterType character_type, TerrainType map[ROWS][COLS], int distance_map[ROWS][COLS], int dist[ROWS][COLS]) {
    ...
    dist[start.get_position_x()][start.get_position_y()] = 0;
    distance_map[start.get_position_x()][start.get_position_y()] = 0;
