...
void pokemon_encountered(int x, int y) {
    if(earth[LY][LX].map[x][y] == TALL_GRASS) {
...
// Sometimes the name is too long and is cut off by Moves.
void display_pokemon_info(const encountered_pokemon p) {
    print_map();
    if(p.get_shiny()) {
        attron(COLOR_PAIR(6));
        mvprintw(0, 0, "Shiny Pokemon encountered!");
        attroff(COLOR_PAIR(6));
    } else {
        mvprintw(0, 0, "Pokemon encountered, move to escape.");
    }
