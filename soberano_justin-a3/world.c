...
int dijkstra_for_pc(Point start, CharacterType character_type, TerrainType m[ROWS][COLS], int dist_map[ROWS][COLS], int dist[ROWS][COLS]) {
        ...
        for (int i = 0; i < 8; i++) {
            ...
            if (is_valid(nx, ny)) {
                ...
                if (character_type == Hiker) {
                    move_cost = hiker_costs[m[nx][ny]];
                } else if (character_type == Rival) {
                    move_cost = rival_costs[m[nx][ny]];
                }
    ...
    for(int i = 0; i < ROWS; i++) {
        for(int j = 0; j < COLS; j++) {
            if(m[i][j] != Player && dist_map[i][j] == 0) {
...
void gen() {
    ...
    dijkstra_for_pc(pc, Hiker, m, dist_map_hiker, dist);
    ...
    dijkstra_for_pc(pc, Rival, m, dist_map_rival, dist);
