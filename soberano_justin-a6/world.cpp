...
enum TrainerType {Hkr, Rvl, Pcr, Wndr, Expl, Stry};
...
enum TerrainType { Border, Tree, Path, PokeMart, CenterMart, TallGrass, ShortGrass, Water, Player };

TerrainType m[ROWS][COLS];

enum CharacterType {Hiker, Rival};
...
void print_gates_for_debugging() {
    mvprintw(21, 0, "North gate: %d", earth[LY][LX].north_gate);
    mvprintw(22, 0, "South gate: %d", earth[LY][LX].south_gate);
    mvprintw(23, 0, "East gate: %d", earth[LY][LX].east_gate);
    mvprintw(24, 0, "West gate: %d", earth[LY][LX].west_gate);
}

void print_player_character_position() {
    mvprintw(15, 0, "POS: %d, %d", pc.x, pc.y);
}
...
// TODO:
// double check fly and map movement and check if the trainers move correctly < Seems good for now.